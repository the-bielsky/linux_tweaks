...
function _autocheck_space(){
    ...
    if [ $? -ne 0 ]; then
        ...
        shout "Disk space checked by $0"
        shout "Not enough disk space in docker disk (${docker_dir})"
        shout "limit is set to ${DOCKER_REQUIRED_DISK_SPACE} KB"
        shout "Please free some space or change the limit in DOCKER_REQUIRED_DISK_SPACE variable"
        return 1
    fi
}
